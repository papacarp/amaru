...
use amaru_kernel::{AsHash, Credential, HasLovelace, Lovelace, PoolId};
...
pub fn calculate_stake_by_pool(
    ...
    let mut stake_by_credential: BTreeMap<Credential, Lovelace> = BTreeMap::new();
    ...
    for (pool_id, pool_data) in stake_by_pool.iter_mut() {
        if let Ok(Some(pool_row)) = db.pool(pool_id) {
            ...
            for owner_hash in &pool_params.owners {
                let owner_credential = Credential::KeyHash(*owner_hash);
...
pub fn calculate_detailed_pool_data(
    ...
    let mut stake_by_credential: BTreeMap<Credential, Lovelace> = BTreeMap::new();
    let mut pool_delegator_stake: BTreeMap<PoolId, BTreeMap<Credential, Lovelace>> = BTreeMap::new();
    ...
    for (pool_id, pool_row) in db.iter_pools()? {
        ...
        for (credential, stake) in &delegators {
            let credential_bytes = credential.as_hash().as_slice().to_vec();
