...
use amaru_kernel::{BlockHeader, IsHeader, Slot};
...
pub fn run(args: Args) -> Result<(), Box<dyn Error>> {
    ...
    let db = RocksDBStore::open_for_readonly(&config)
        .map_err(|e| format!(
    ...
    // Get the tip hash
    let tip_hash = db.get_best_chain_hash();
    ...
    let era_history = detected_network
        .as_era_history()
        .ok_or_else(|| format!("no era history available for network {detected_network}"))?
        .clone();
