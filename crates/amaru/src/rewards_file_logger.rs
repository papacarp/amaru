...
/// 
/// Creates a separate file per epoch in subdirectory: `{base_parent}/{epoch}/rewards.csv`
/// Writes a terminal marker `0,0,0` when rewards calculation for an epoch completes.
/// 
/// Also creates a summary file `{base_parent}/{epoch}/summary.json` for each epoch containing
...
impl RewardsFileLogger {
    ...
    fn get_summary_file_path(&self, epoch: u64) -> PathBuf {
        ...
        parent.join(epoch.to_string()).join("summary.json")
    }
    ...
    fn get_epoch_file_path(&self, epoch: u64) -> PathBuf {
        ...
        let name = self.base_path.file_stem()
            .and_then(|n| n.to_str())
            .unwrap_or("rewards");
        parent.join(epoch.to_string()).join(format!("{}.csv", name))
    }
